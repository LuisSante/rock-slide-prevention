...
void velocidad(float vertices_talud_[], float Xd, float Yd, float Xc, float Yc, float vx, float vy, float w)
{
    float r1 = (Xd - Xc) * (Xd - Xc);
    float r2 = (Yd - Yc) * (Yd - Yc);
    float rc = sqrt(r1 + r2);
    ...
    float vc_i = vx - (w * rc * sin(angle));
    float vc_j = vy + (w * rc * cos(angle));
